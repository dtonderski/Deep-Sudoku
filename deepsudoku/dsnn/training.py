...
def generate_training_data(
    train_sudokus: List[Tuple[np.ndarray, np.ndarray, bool]],
    ...
    for sudoku_package in sudokus:
        ...
        if not successful_game:
            ...
            for encountered_state in node.encountered_states:
                valid = np.all(
                    np.logical_or(
                        encountered_state == 0, encountered_state == solution
                    )
                )
            ...
            if verbose:
                if len(saved_states) // (min_data_size // 8) >= print_j:
                    print_j += 1
                    print(
                        f"Sampled {len(saved_states)}/{min_data_size} "
                        f"sudokus, time = {datetime.now()}"
                    )

            if verbose > 1:
                print(
                    "Current valids fraction: "
    ...
    if verbose:
        print(
            f"Valids fraction: {sum(valids) / len(valids):.2f}, "
