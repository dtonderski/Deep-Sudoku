...
def evaluate(
    ...
    for i, (sudoku_board, solution, _) in enumerate(sudoku_packages):
        ...
        moves_before_ending = starting_blanks if successful_game else len(
            node.action_set) - 1
...
def get_averages(moves_before_ending_dict: Dict[int, List[int]],
...
def print_evaluation(avg_moves_dict: Dict[int, float],
                     avg_percentage_dict: Dict[int, float]) -> None:
