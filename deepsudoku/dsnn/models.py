...
from deepsudoku.dsnn import se_resnet, sudoker, transformer
...
def load_model(
    model_name: Literal["SeResNet", "ViTTiTransformer", "ViTTiSudoker"],
    ...
    if use_builtin_checkpoint:
        if model_name != "ViTTiSudoker":
            print(
                "Not loading built-in checkpoint - checkpoint not included for"
                f" {model_name=}!"
            )
            return model
        with importlib.resources.path(
            "deepsudoku.resources", "ViTTiSudoker.pth"
        ) as checkpoint_path:
